...
internal static class InventoryReceivedPdfReport
{
    public static byte[] BuildPdf(InventoryReceivedReportModel model)
    {
    ...
    private static void ComposeHeader(IContainer container, InventoryReceivedReportModel model)
    {
        container.Table(table =>
        {
            ...
            {
                c.Item().AlignCenter().Text("ITEM RECEIVED VOUCHER").FontFamily("Lato").FontSize(16).Bold();
                c.Item().PaddingTop(2).AlignCenter().Text("PHIẾU NHẬP HÀNG").FontFamily("Lato").FontSize(16).Bold();
            });
            table.Cell().AlignRight().Text(model.FlowDateText).Bold();
            table.Cell().PaddingTop(8).Text($"Receive House: {model.StoreName}");
            table.Cell().PaddingTop(8).AlignCenter().Text($"PO No: {model.PONo}");
            table.Cell().PaddingTop(8).AlignRight().Text($"Supplier: {model.SupplierName}");
    ...
    private static void ComposeDetails(IContainer container, InventoryReceivedReportModel model)
    {
        container.Table(table =>
        {
            table.ColumnsDefinition(columns =>
    ...
    private static void ComposeSignatures(IContainer container, InventoryReceivedReportModel model)
    {
        container.Row(row =>
        {
            row.Spacing(24);
            row.RelativeItem().Element(c => ComposeSignatureBlock(c, "Store Man", model.Level2Signature));
            row.RelativeItem().Element(c => ComposeSignatureBlock(c, "Check & Confirm by", model.Level3Signature));
            row.RelativeItem().Element(c => ComposeSignatureBlock(c, "Confirm by", model.Level4Signature));
        });
    }
...
internal sealed class InventoryReceivedReportModel
{
    ...
    public byte[]? Level2Signature { get; set; }
    public byte[]? Level3Signature { get; set; }
    public byte[]? Level4Signature { get; set; }
    public List<InventoryReceivedReportItem> Items { get; set; } = new();
}

internal sealed class InventoryReceivedReportItem
{
